<template>
  <th v-if="!props.column?.resizable || !props.column?.width" v-bind="$attrs" @clickCapture="onClickCapture">
    <slot></slot>
  </th>
  <th
    v-else
  ...
  >
    <slot></slot>
    <span :class="['resizable-handle', { dragging: dragging }]" @mousedown="onMouseDown"></span>
  </th>
</template>
